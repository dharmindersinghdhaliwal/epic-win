...
function dpa_achievement_custom_column( $column, $post_id ) {		
	...
	else if( 'acategories' === $column )
	{
		$term_list = wp_get_post_terms($post_id, 'acategories', array("fields" => "all"));
		
		if( !empty( $term_list ) )
		{
			echo '<a href="edit.php?post_type=achievement&amp;category_name='.$term_list[0]->slug.'">'.$term_list[0]->name.'</a>';
		}
	}
}
