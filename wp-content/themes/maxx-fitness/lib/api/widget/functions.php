 ...
 *     @type string $maxxfitness_type                 Optional. The widget area type. Accepts 'stack', 'grid' or 'offcanvas'.
 *                                           Default stack.
 *     @type bool   $maxxfitness_show_widget_title    Optional. Whether to show the widget title or not. Default true.
 *     @type bool   $maxxfitness_show_widget_badge    Optional. Whether to show the widget badge or not. Default false.
 *     @type bool   $maxxfitness_widget_badge_content Optional. The badge content. This may contain widget shortcodes
 *                                           {@see maxxfitness_widget_shortcodes()}. Default 'Hello'.
 * }
 * @return string The widget area ID is added to the $wp_registered_sidebars globals when the widget area is setup.
 */
function maxxfitness_register_widget_area( $args = array(), $widget_control = array() ) {

    // Stop here if the id isn't set.
    if ( !$id = maxxfitness_get( 'id', $args ) )
        return;
     ...
     * @since 1.0.0
     */
    $defaults = apply_filters( 'maxxfitness_widgets_area_default_args', array(
        'maxxfitness_type' => 'stack',
        'maxxfitness_show_widget_title' => true,
        'maxxfitness_show_widget_badge' => false,
        'maxxfitness_widget_badge_content' => esc_html__( 'Hello', 'maxx-fitness' )
    ) );
     ...
     * @since 1.0.0
     */
    $args = maxxfitness_apply_filters( "maxxfitness_widgets_area_args[_{$id}]", array_merge( $defaults, $args ) );
 ...
 */
function maxxfitness_deregister_widget_area( $id ) {
 ...
 */
function maxxfitness_is_active_widget_area( $id ) {
 ...
 *
 * While {@see maxxfitness_is_active_widget_area()} checks if a widget area contains widgets, this function only checks if a widget
 * area is registered.
 *
 * @since 1.0.0
 *
 ...
 */
function maxxfitness_has_widget_area( $id ) {
 ...
 */
function maxxfitness_widget_area( $id ) {

    // Stop here if the widget area is not registered.
    if ( !maxxfitness_has_widget_area( $id ) )
        return false;

    maxxfitness_tt_setup_widget_area( $id );
     ...
     * @since 1.0.0
     */
    do_action( 'maxxfitness_widget_area_init' );

        ob_start();

            /**
             * Fires when {@see maxxfitness_widget_area()} is called.
             *
             * @since 1.0.0
             */
            do_action( 'maxxfitness_widget_area' );

        $output = ob_get_clean();

    // Reset widget area global to reduce memory usage.
    maxxfitness_tt_reset_widget_area();
     ...
     * @since 1.0.0
     */
    do_action( 'maxxfitness_widget_area_reset' );
 ...
 */
function maxxfitness_get_widget_area( $needle = false ) {

    global $maxxfitness_tt_widget_area;

    if ( !$needle )
        return $maxxfitness_tt_widget_area;

    return maxxfitness_get( $needle, $maxxfitness_tt_widget_area );
 ...
 */
function maxxfitness_widget_area_shortcodes( $content ) {

    if ( is_array( $content ) )
        $content = build_query( $content );

    return maxxfitness_array_shortcodes( $string, $GLOBALS['maxxfitness_tt_widget_area'] );
 ...
 */
function maxxfitness_have_widgets() {

    global $maxxfitness_tt_widget_area;

    if ( !maxxfitness_get( 'widgets', $maxxfitness_tt_widget_area ) )
        return false;

    $widgets = array_keys( $maxxfitness_tt_widget_area['widgets'] );

    if ( isset( $widgets[$maxxfitness_tt_widget_area['current_widget']] ) )
        return true;

    // Reset last widget global to reduce memory usage.
    maxxfitness_tt_reset_widget();
 ...
 */
function maxxfitness_setup_widget() {
    ...
    if ( !$id = maxxfitness_get( $maxxfitness_tt_widget_area['current_widget'], $widgets ) )
         return false;

    // Set next current widget integer.
    $maxxfitness_tt_widget_area['current_widget'] = $maxxfitness_tt_widget_area['current_widget'] + 1;

    maxxfitness_tt_setup_widget( $id );
 ...
 */
function maxxfitness_get_widget( $needle = false ) {

    global $maxxfitness_tt_widget;

    if ( !$needle )
        return $maxxfitness_tt_widget;

    return maxxfitness_get( $needle, $maxxfitness_tt_widget );
 ...
 * @return string Content with shortcodes filtered out.
 */
function maxxfitness_widget_shortcodes( $content ) {

    if ( is_array( $content ) )
        $content = build_query( $content );

    return maxxfitness_array_shortcodes( $content, $GLOBALS['maxxfitness_tt_widget'] );
 ...
 * @ignore
 */
function maxxfitness_tt_setup_widget_area( $id ) {

    global $maxxfitness_tt_widget_area, $wp_registered_sidebars;
    ...
    $maxxfitness_tt_widget_area = $wp_registered_sidebars[$id];

    // Buffer sidebar, please make it easier for us wp.
    $sidebar = maxxfitness_render_function( 'dynamic_sidebar', $id );
    ...
    $maxxfitness_tt_widget_area['widgets_count'] = count( $counter[0] );
    $maxxfitness_tt_widget_area['current_widget'] = 0;

    // Only add widgets if exists.
    if ( count( $splited_sidebar ) == 3 ) {

        $maxxfitness_tt_widget_area['before_widgets'] = $splited_sidebar[0];
        $maxxfitness_tt_widget_area['widgets'] = maxxfitness_tt_setup_widgets( $splited_sidebar[1] );
        $maxxfitness_tt_widget_area['after_widgets'] = $splited_sidebar[2];
 ...
 * @ignore
 */
function maxxfitness_tt_setup_widgets( $widget_area_content ) {

    global $wp_registered_widgets, $maxxfitness_tt_widget_area;

    $maxxfitness_tt_widgets = array();
    ...
    foreach ( explode( '<!--widget-end-->', $widget_area_content ) as $content ) {
        ...
        if ( !$data = maxxfitness_get( $id, $wp_registered_widgets ) )
            continue;
        ...
        // Add total count.
        $widget['count'] = $maxxfitness_tt_widget_area['widgets_count'];
        ...
        $maxxfitness_tt_widgets[$widget['id']] = array_merge( $widget, array(
            'show_title' => $maxxfitness_tt_widget_area['maxxfitness_show_widget_title'],
            'badge' => $maxxfitness_tt_widget_area['maxxfitness_show_widget_badge'],
            'badge_content' => $maxxfitness_tt_widget_area['maxxfitness_widget_badge_content'],
        ) );

    }

    return $maxxfitness_tt_widgets;
 ...
 * @ignore
 */
function maxxfitness_tt_setup_widget( $id ) {

    global $maxxfitness_tt_widget;

    $widgets = maxxfitness_get_widget_area( 'widgets' );

    $maxxfitness_tt_widget = $widgets[$id];
 ...
 * @ignore
 */
function maxxfitness_tt_reset_widget_area() {

    unset( $GLOBALS['maxxfitness_tt_widget_area'] );
 ...
 * @ignore
 */
function maxxfitness_tt_reset_widget() {

    unset( $GLOBALS['maxxfitness_tt_widget'] );
 ...
 * @ignore
 */
function maxxfitness_tt_widget_area_subfilters() {

    global $maxxfitness_tt_widget_area;

    // Add sidebar id.
    return '[_' . $maxxfitness_tt_widget_area['id'] . ']';
 ...
 * @ignore
 */
function maxxfitness_tt_widget_subfilters() {

    global $maxxfitness_tt_widget_area, $maxxfitness_tt_widget;

    $subfilters = array(
        $maxxfitness_tt_widget_area['id'], // Add sidebar id.
        $maxxfitness_tt_widget['type'], // Add widget type.
        $maxxfitness_tt_widget['id'] // Add widget id.
    );
...
add_action( 'the_widget', 'maxxfitness_tt_force_the_widget', 10, 3 );
 ...
 * @ignore
 */
function maxxfitness_tt_force_the_widget( $widget, $instance, $args ) {
    ...
    if ( stripos( $widget_obj->id, maxxfitness_get( 'before_widget', $args ) ) !== false )
        return;
