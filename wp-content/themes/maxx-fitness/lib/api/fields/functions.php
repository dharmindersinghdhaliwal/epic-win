 ...
 */
function maxxfitness_register_fields( array $fields, $context, $section ) {

	if ( empty( $fields ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
	require_once( maxxfitness_API_PATH . 'fields/class.php' );

	$class = new maxxfitness_tt_Fields();
 ...
 */
function maxxfitness_get_fields( $context, $section = false ) {

	if ( !class_exists( 'maxxfitness_tt_Fields' ) )
		return;
 ...
 * {@see maxxfitness_get_fields()}.
 *
 * @since 1.0.0
 *
 * @param array $field Array of data obtained using {@see maxxfitness_get_fields()}.
 */
function maxxfitness_field( $field ) {
 ...
 * @ignore
 */
function maxxfitness_tt_pre_standardize_fields( $fields ) {
	...
	foreach ( $fields as $field ) {
		...
		if ( maxxfitness_get( 'type', $field ) === 'group' )
			$_fields[$field['id']]['fields'] = maxxfitness_tt_pre_standardize_fields( $field['fields'] );
