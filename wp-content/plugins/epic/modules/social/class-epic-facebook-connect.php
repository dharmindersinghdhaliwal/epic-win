...
class epic_Facebook_Connect extends epic_Social_Connect{
	...
	public function login(){
        ...
        $facebook = new Facebook\Facebook([
          'app_id' => $this->epic_settings['social_login_facebook_app_id'],
          'app_secret' => $this->epic_settings['social_login_facebook_app_secret'],
...
//			$loginUrl = $facebook->getLoginUrl(array('redirect_uri'=>$callback_url, 'scope'=>'email'));
            ...
            try {
              $accessToken = $helper->getAccessToken();
				...
				$response->error_message = epic_language_entry('Graph returned an error:');

				$this->handle_social_error('Facebook',$response->error_code);
            } catch(Facebook\Exceptions\FacebookSDKException $e) {
                 $response->error_code 	= 'auth_invalid';
				$response->error_message = epic_language_entry('Facebook SDK returned an error:');
