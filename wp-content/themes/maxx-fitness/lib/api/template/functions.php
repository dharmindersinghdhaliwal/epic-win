 ...
 * @since 1.0.0
 */
function maxxfitness_load_document() {
	 ...
	 * @since 1.0.0
	 */
	do_action( 'maxxfitness_before_load_document' );
		 ...
		 * @since 1.0.0
		 */
		do_action( 'maxxfitness_load_document' );
	 ...
	 * @since 1.0.0
	 */
	do_action( 'maxxfitness_after_load_document' );
 ...
 */
function maxxfitness_load_default_template( $file ) {

	$file = maxxfitness_STRUCTURE_PATH . basename( $file );
 ...
 */
function maxxfitness_load_fragment_file( $slug ) {
	 ...
	 */
	if ( apply_filters( 'maxxfitness_pre_load_fragment_' . $slug, false ) )
		return false;

	// Stop here if fragment file doesn't exists.
	if ( !file_exists( maxxfitness_FRAGMENTS_PATH . $slug . '.php' ) )
		return false;

	require_once( maxxfitness_FRAGMENTS_PATH . $slug . '.php' );
 ...
 * @since 1.0.0
 */
function maxxfitness_comment_callback( $comment, $args, $depth ) {
