...
<table id="wpupg_form_data_source" class="wpupg_form">
    <tr>
        <td><label for="wpupg_epost_types"><?php _e( 'Post Types', 'wp-eultimate-post-grid' ); ?></label></td>
        <td>
            <select name="wpupg_epost_types" id="wpupg_epost_types" class="wpupg-select2">
                <?php
                ...
                foreach( $post_types as $post_type => $options ) {
                    $selected = in_array( $post_type, $egrid->post_types() ) ? ' selected="selected"' : '';
        ...
        <td>
            <select name="wpupg_order_by" id="wpupg_order_by" class="wpupg-select2">
                ...
                foreach( $order_by_options as $order_by => $order_by_name ) {
                    $selected = $order_by == $egrid->order_by() ? ' selected="selected"' : '';
        ...
        <td>
            <select name="wpupg_order" id="wpupg_order" class="wpupg-select2">
                ...
                foreach( $order_options as $order => $order_name ) {
                    $selected = $order == $egrid->order() ? ' selected="selected"' : '';
    ...
    <tr class="wpupg_divider">
        ...
        <td>
            <input type="checkbox" name="wpupg_images_only" id="wpupg_images_only" <?php if( $egrid->images_only() ) echo 'checked="true" '?>/>
        </td>
    ...
    <tr class="wpupg_divider">
        ...
        <td>
            <input type="checkbox" name="wpupg_limit_posts" id="wpupg_limit_posts" <?php if( $egrid->limit_posts() ) echo 'checked="true" '?>/>
