 ...
 */
function maxxfitness_edit_image( $src, array $args, $output = 'STRING' ) {

	require_once( maxxfitness_API_PATH . 'image/class-images.php' );

	$instance = new maxxfitness_tt_Image_Editor( $src, $args, $output );
 ...
 */
function maxxfitness_get_post_attachment( $post_id, $size = 'full' ) {
 ...
 *
 * This function is shortuct of {@see maxxfitness_edit_image()}. It should be used to edit a post attachment.
 *
 * @since 1.0.0
 *
 * @param string $post_id The post id.
 ...
 */
function maxxfitness_edit_post_attachment( $post_id, $args = array() ) {

	if ( !has_post_thumbnail( $post_id ) )
		return false;

	// Get full size image.
	$attachement = maxxfitness_get_post_attachment( $post_id, 'full' );

	if ( !$edited = maxxfitness_edit_image( $attachement->src, $args, 'ARRAY_A' ) )
		return $attachement;
 ...
 */
function maxxfitness_get_images_dir() {
	 ...
	 * @since 1.0.0
	 */
	$dir = apply_filters( 'maxxfitness_images_dir', trailingslashit( $wp_upload_dir['basedir'] ) . 'beans/images/' );
