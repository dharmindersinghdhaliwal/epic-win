...
class epic_Import_Export
{
    ...
    public function __construct()
    {
        ...
        if (is_admin()) {
            ...
            add_action('wp_ajax_epic_download_export_fields', array($this, 'epic_download_export_fields'));
            add_action('wp_ajax_epic_upload_import_fields', array($this, 'epic_upload_import_fields'));

            /* AJAX requests for import and export for settings */
            add_action('wp_ajax_epic_download_export_settings', array($this, 'epic_download_export_settings'));
            add_action('wp_ajax_epic_upload_import_settings', array($this, 'epic_upload_import_settings'));
            
            /* AJAX requests for import and export for users */
            add_action('wp_ajax_epic_download_export_users', array($this, 'epic_download_export_users'));
    ...
    public function epic_upload_import_fields()
    {
        global $epic_admin;
        if (epic_is_in_get('current_tab')) {
            ...
            if (isset($_FILES['fields_file']) && $_FILES['fields_file']['error'] == 0) {
                ...
                if (!in_array(strtolower($_FILES['fields_file']['type']), $allowed)) {
                    ...
                    $optionArray = get_option('epic_profile_fields');

                    $updatedArray = array();

                    if (epic_is_in_post('site_import_fields') && count(epic_post_value('site_import_fields') > 0)) {
                        //get option array

                        foreach (epic_post_value('site_import_fields') as $postedVal) {
                    ...
                    update_option('epic_profile_fields', ($mergedArray));
    ...
    public function epic_download_export_fields()
    {
        global $epic_admin;

        if (epic_is_in_get('current_tab')) {

            $optionsRaw = get_option('epic_profile_fields');
            if (epic_is_in_get('site_export_field_type')) {
                if (epic_get_value('site_export_field_type') == 'selected_fields') {

                    $options = array();

                    if (epic_is_in_get('site_export_fields') && count(epic_get_value('site_export_fields')) > 0) {

                        foreach ($optionsRaw as $key => $value) {
                            if (in_array($key, epic_get_value('site_export_fields'))) {
                                $options[$key] = $value;
                            }
                        }
...
//echo "<pre>";print_r($exp_val);
    ...
    public function epic_upload_import_settings()
    {
        global $epic_admin;
        if (epic_is_in_get('current_tab')) {
            ...
            if (isset($_FILES['settings_file']) && $_FILES['settings_file']['error'] == 0) {
                ...
                if (!in_array(strtolower($_FILES['settings_file']['type']), $allowed)) {
                    ...
                    $optionArray = get_option('epic_options');

                    $updatedArray = array();

                    if (epic_is_in_post('site_import_settings') && count(epic_post_value('site_import_settings') > 0)) {
                        //get option array

                        foreach (epic_post_value('site_import_settings') as $postedVal) {
                    ...
                    update_option('epic_options', ($mergedArray));
    ...
    public function epic_download_export_settings()
    {
        global $epic_admin;

        if (epic_is_in_get('current_tab')) {

            $optionsRaw = get_option('epic_options');
            if (epic_is_in_get('site_export_settings_type')) {
                if (epic_get_value('site_export_settings_type') == 'selected_settings') {
    ...
    public function epic_download_export_users()
    {
        global $epic_admin;

        if (epic_is_in_get('current_tab')) {

            $profileFieldsRaw = get_option('epic_profile_fields');
            if (epic_is_in_get('site_export_users_type')) {
                if (epic_get_value('site_export_users_type') == 'all_users') {
                    ...
                    $user = (array) epic_get_value('site_export_users_type');
                    $users_query = new WP_User_Query( array( 
                        'orderby' => 'registered',
                        'order'   => 'desc',
                        'include' => $user
...
$epic_import_export = new epic_Import_Export();
