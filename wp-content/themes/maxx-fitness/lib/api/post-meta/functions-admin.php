 ...
 *            					  {@see maxxfitness_register_fields()} $fields arguments.
 ...
 */
function maxxfitness_register_post_meta( array $fields, $conditions, $section, $args = array() ) {

	global $maxxfitness_tt_post_meta_conditions;
	 ...
	 */
	$fields = apply_filters( "maxxfitness_post_meta_fields_{$section}", maxxfitness_tt_pre_standardize_fields( $fields ) );
	 ...
	 */
	$conditions = apply_filters( "maxxfitness_post_meta_post_types_{$section}", $conditions );
	$maxxfitness_tt_post_meta_conditions = array_merge( $maxxfitness_tt_post_meta_conditions, (array) $conditions );

	// Stop here if the current page isn't concerned.
	if ( !maxxfitness_tt_is_post_meta_conditions( $conditions ) || !is_admin() )
		return;

	// Stop here if the field can't be registered.
	if ( !maxxfitness_register_fields( $fields, 'post_meta', $section ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
	require_once( maxxfitness_API_PATH . 'post-meta/class.php' );

	new maxxfitness_tt_Post_Meta( $section, $args );
 ...
 * @ignore
 */
function maxxfitness_tt_is_post_meta_conditions( $conditions ) {

	// Check if it is a new post and treat it as such.
	if ( stripos( $_SERVER['REQUEST_URI'], 'post-new.php' ) !== false ) {

		if ( !$current_post_type = maxxfitness_get( 'post_type' ) )
		...
		if ( $id = maxxfitness_get( 'post' ) )
			$post_id = $id;
		// Try to get id from $_POST.
		elseif ( $id = maxxfitness_post( 'post_ID' ) )
			$post_id = $id;
...
add_action( 'admin_print_footer_scripts', 'maxxfitness_tt_post_meta_page_template_reload' );
 ...
 * @ignore
 */
function maxxfitness_tt_post_meta_page_template_reload() {

	global $maxxfitness_tt_post_meta_conditions, $pagenow;

	// Stop here if not editing a post object.
	if ( !in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) )
		return;

	$encode = json_encode( $maxxfitness_tt_post_meta_conditions );
...
if ( !isset( $maxxfitness_tt_post_meta_conditions ) )
	$maxxfitness_tt_post_meta_conditions = array();