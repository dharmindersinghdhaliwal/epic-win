...
if( $query->have_posts() ) {
    ...
    foreach( $posts as $post ) {
        ...
        $post_content .= preg_replace("/\[wpuep-searchable-exercise\][^\[]*\[\/wpuep-searchable-exercise\]/", "", $post->post_content);
        ...
        update_post_meta( $exercise->ID(), 'wpuep_text_search_2', time() );
    }
} else {
    // Finished migrating, all exercises have a full text search
    update_option( 'wpuep_cron_migrate_version', '2.3.1' );
}