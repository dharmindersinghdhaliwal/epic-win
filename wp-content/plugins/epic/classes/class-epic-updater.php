 ...
 *
 */
add_action('admin_init', 'epic_upgrade_routine');

function epic_upgrade_routine() {

 
    $stored_version = get_option('epic_version');
    $current_version = epic_get_plugin_version();

    if (!$stored_version && $current_version) {
        epic_initialize_regular_tasks();
        update_option('epic_version', $current_version);
    }
...
function epic_upgrade_1_8() {

    if (empty($GLOBALS['wp_rewrite'])) {
        $GLOBALS['wp_rewrite'] = new WP_Rewrite();
    }

    // Getting current epic Options
    $current_option = get_option('epic_options');



    if (!isset($current_option['profile_page_id']) || $current_option['profile_page_id'] == 0) {
        // Get default page created by epic of earlier version
        $id = get_option('epic_profile_page');
    ...
    if (!isset($current_option['registration_page_id']) || $current_option['registration_page_id'] == 0) {
        ...
        $reg_data = array(
            'post_title' => __('Register', 'epic'),
            'post_type' => 'page',
            'post_name' => 'register',
            'post_content' => '[epic_registration]',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'ping_status' => 'closed',
            'post_author' => 1
    ...
    if (!isset($current_option['login_page_id']) || $current_option['login_page_id'] == 0) {
        ...
        $login_data = array(
            'post_title' => __('Login', 'epic'),
            'post_type' => 'page',
            'post_name' => 'login',
            'post_content' => '[epic_login]',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'ping_status' => 'closed',
            'post_author' => 1
    ...
    if (!isset($current_option['html_registration_disabled']) || (isset($current_option['html_registration_disabled']) && $current_option['html_registration_disabled'] == ''))
        $current_option['html_registration_disabled'] = __('User registration is currently not allowed.', 'epic');

    if (!isset($current_option['captcha_label']) || (isset($current_option['captcha_label']) && $current_option['captcha_label'] == ''))
        $current_option['captcha_label'] = __('Human Check', 'epic');
    ...
    // Updating epic Option
    update_option('epic_options', $current_option);
}

// Version 2.0.0 upgrade routine
function epic_upgrade_2_0() {
    // Uploader folder upgrade routine 
    $epic_current_upload_path = epic_path . "uploads";
    if (is_dir($epic_current_upload_path)) {
        ...
        $epic_upgraded_upload_path = $upload_base_dir . "/epic/";
        $epic_upgraded_upload_url = $upload_dir['baseurl'] . "/epic/";

        $epic_current_upload_path = epic_path . "uploads/";
        $epic_current_upload_url = epic_url . "uploads/";

        // Check the existence of epic folder within uploads folder of the site
        if (wp_mkdir_p($epic_upgraded_upload_path)) {


            $source = $epic_current_upload_path;
            $destination = $epic_upgraded_upload_path;

            // Get array of  uploaded files
            $files = scandir($epic_current_upload_path);
            $files_delete = array();
            ...
            if (($available_files == $delete_files) && (0 != $available_files)) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (isset($field['field']) && isset($field['meta']) && 'fileupload' == $field['field']) {
                        ...
                        $result = $wpdb->query(
                                        $wpdb->prepare($sql, $epic_current_upload_url, $epic_upgraded_upload_url, $field['meta'])
                        );
                    }
                }
                // Remove upload directory once all the files have been transfered
                rmdir($epic_current_upload_path);
            }
        } else {
            echo '<p class="error">' . __('Upload folder creation failed.', 'epic') . '</p>';
        }
    }
}

function epic_upgrade_2_0_1() {
    ...
    //Adding epic Setting for Cron Usage
    ...
    foreach ($profile_fields as $key => $value) {
        ...
        if ($profile_fields[$key]['type'] == 'separator') {
            $profile_fields[$key]['meta'] = epic_manage_string_for_meta($value['name']) . '_separator';
    ...
    update_option('epic_profile_fields', $profile_fields);

    /* Upgrade Routine to Create Cache for Meta Values for All Users */
    $users = get_users(array('fields' => 'ID'));

    foreach ($users as $key => $value) {
        epic_update_user_cache($value);
    }
}

function epic_upgrade_2_0_2() {
    // Adding epic Setting for Hide Admin Bar on Frontend
    $current_option = get_option('epic_options');
...
function epic_upgrade_2_0_4() {
    flush_rewrite_rules();    
    
}

function epic_upgrade_2_0_5() {

    $current_option = get_option('epic_options');
    $current_option['require_search_input'] = '0';
    $current_option['users_are_called'] = __('Users','epic');
    $current_option['combined_search_text'] = __('Combined Search','epic');
    $current_option['search_button_text'] = __('Filter','epic');
    $current_option['profile_title_field'] = 'display_name';

    update_option('epic_options', $current_option);
}

function epic_initialize_regular_tasks(){
   flush_rewrite_rules(); 
}

function epic_upgrade_2_0_6() {
    global $wpdb;

    $user_query = new WP_User_Query( array( 
        'meta_key' => 'epic_activation_status', 
    ...
    foreach ($users as $key => $user) {
        ...
        update_user_meta($user->data->ID, 'epic_activation_status', "ACTIVE");
        update_user_meta($user->data->ID, 'epic_activation_code',$activation_code);
    }

    $current_option = get_option('epic_options');
    $current_option['select_user_role_in_registration'] = '0';
    $current_option['choose_roles_for_registration'] = get_option('default_role');
    $current_option['label_for_registration_user_role'] = __('Select Role', 'epic');
    update_option('epic_options', $current_option);

}

function epic_upgrade_2_0_7() {
    $current_option = get_option('epic_options');
    $current_option['reset_button_text'] = __('Reset', 'epic');
    update_option('epic_options', $current_option);    
}

function epic_upgrade_2_0_8() {
    ...
    if (!isset($current_option['reset_password_page_id']) || $current_option['reset_password_page_id'] == 0) {
        ...
        $reset_password_data = array(
            'post_title' => __('Reset Password', 'epic'),
            'post_type' => 'page',
            'post_name' => 'reset_password',
            'post_content' => '[epic_reset_password]',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'ping_status' => 'closed',
            'post_author' => 1
...
function epic_upgrade_2_0_10() {
    ...
    if (!isset($current_option['member_list_page_id']) || $current_option['member_list_page_id'] == 0) {
        ...
        $member_list_data = array(
            'post_title' => __('Member List', 'epic'),
            'post_type' => 'page',
            'post_name' => 'member_list',
            'post_content' => '[epic_search] [epic group=all view=compact users_per_page=10]',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'ping_status' => 'closed',
            'post_author' => 1
...
function epic_upgrade_2_0_12() {
    global $wpdb;

    $current_option = get_option('epic_options');
    $current_option['choose_roles_for_view_profile'] = 'administrator';
    $current_option['profile_modal_window_shortcode'] = '[epic]';

    //Adding epic Setting for disabling fancybox scripts
    ...
    $current_option['html_other_profiles_restricted'] = __('Viewing of other profiles restricted for your user role.','epic');

    update_option('epic_options', $current_option);
}

function epic_upgrade_2_0_14() {
    global $wpdb;
    ...
    $current_option['html_profile_status_msg'] = __('This is a private profile. You are not allowed to view this profile.', 'epic');
    $current_option['html_profile_approval_pending_msg'] = __('This profile is pending approval. You will get a notification once the profile is approved.', 'epic');
    $current_option['profile_approval_status'] = '0';
    $current_option['ajax_profile_field_save'] = '0';

    $current_option['html_terms_and_conditions'] = __('I agree to Terms and Conditions.', 'epic');
...
function epic_upgrade_2_0_17() {
    ...
    $current_option['html_members_private_content']     = __('This content is restricted for your user account.', 'epic');
    
    update_option('epic_options', $current_option);
}


function epic_upgrade_2_0_20() {
    global $epic_email_templates;
    $epic_email_templates->epic_reset_all_templates();
}

function epic_upgrade_2_0_21() {
    $current_option = get_option('epic_options');

    $current_option['link_post_author_to_epic']  = '0';
    $current_option['display_profile_after_post']  = '0';
    update_option('epic_options', $current_option);
}

function epic_upgrade_2_0_22() {
    $current_option = get_option('epic_options');
...
function epic_upgrade_2_0_23(){
    global $wpdb,$epic_email_templates;
    ...
    $email_templates['two_factor_email_verify'] = $epic_email_templates->epic_get_template('two_factor_email_verify','1');
    update_option('epic_email_templates',$email_templates);

}

function epic_upgrade_2_0_24(){
    global $wpdb,$epic_email_templates;
    ...
    $current_option['email_from_name'] = __('WordPress','epic');
    $current_option['email_from_address'] = epic_get_default_email_address();
    ...
    $email_templates = get_option('epic_email_templates');
    $email_templates['reg_activation_approval_admin'] = $epic_email_templates->epic_get_template('reg_activation_approval_admin','1');
    $email_templates['reg_activation_approval_user'] = $epic_email_templates->epic_get_template('reg_activation_approval_user','1');
    $email_templates['reg_activation_admin'] = $epic_email_templates->epic_get_template('reg_activation_admin','1');
    $email_templates['reg_activation_user'] = $epic_email_templates->epic_get_template('reg_activation_user','1');
    $email_templates['reg_approval_admin'] = $epic_email_templates->epic_get_template('reg_approval_admin','1');
    $email_templates['reg_approval_user'] = $epic_email_templates->epic_get_template('reg_approval_user','1');
    
    update_option('epic_email_templates',$email_templates);
}

function epic_upgrade_2_0_25(){
    global $epic_email_templates;
    $current_option = get_option('epic_options');
    $current_option['help_text_html'] = '0';
    $current_option['register_form_title_text'] = __('Your display name will appear here.','epic');
    ...
    $email_templates = get_option('epic_email_templates');
    $email_templates['delete_profile_confirm'] = $epic_email_templates->epic_get_template('delete_profile_confirm','1');    
    update_option('epic_email_templates',$email_templates);
}

function epic_upgrade_2_0_29(){
    global $epic_email_templates;
    $current_option = get_option('epic_options');
