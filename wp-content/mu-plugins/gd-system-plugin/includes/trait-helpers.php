...
trait Helpers {
	...
	public static function use_nextgen() {
		...
		$has_go_theme_template = ( 'go' === get_option( 'stylesheet' ) && self::has_used_wpnux() );
		$has_wordpress_55      = version_compare( $wp_version, '5.5', '>=' );
		// If this helper method is being called AFTER the 'plugins_loaded' hook then check for
		// the minimum required Gutenberg version, otherwise just check if Gutenberg is active.
		...
		$has_nextgen_constant  = defined( 'GD_NEXTGEN' ) ? (bool) GD_NEXTGEN : false;

		$result = ( $has_go_theme_template && ( $has_wordpress_55 || $has_gutenberg_plugin ) && ( $has_nextgen_query_arg || $has_nextgen_constant ) );
		 ...
		 * @var bool
		 */
		return (bool) apply_filters( 'wpaas_use_nextgen', $result );
