...
class WPUPG_Eassets {
    ...
    {
        if( !is_admin() ) {
            $base_layout = WPUltimateEPostGrid::option( 'grid_template_force_style', '1' ) == '1' ? 'layout_base_forced.css' : 'layout_base.css';
            ...
            $this->add(
                ...
                array(
                    ...
                    'data' => array(
                        ...
                        'animationSpeed' => WPUltimateEPostGrid::option( 'grid_animation_speed', '0.8' ) . 's',
                        'nonce' => wp_create_nonce( 'wpupg_egrid' ),
                    ),
                )
            );
        }
        ...
        foreach( $assets as $asset )
        {
            if( isset( $asset['file'] ) ) {
                ...
                if( isset( $asset['direct'] ) && $asset['direct'] ) {
                    ...
                    $base_url = WPUltimateEPostGrid::get()->coreUrl;
                    $base_dir = WPUltimateEPostGrid::get()->coreDir;

                    if( isset( $asset['premium'] ) && $asset['premium'] ) {
                        $base_url = WPUltimateEPostGridPremium::get()->premiumUrl;
                        $base_dir = WPUltimateEPostGridPremium::get()->premiumDir;
                    }
        ...
        foreach( $assets as $asset )
        {
            ...
            if( isset( $asset['page'] ) ) {
                ...
                switch ( strtolower( $asset['page'] ) ) {
                    ...
                    case 'grid_posts':
                        if( $hook != 'edit.php' || $screen->post_type != WPUPG_EPOST_TYPE ) continue 2; // Switch is consider a loop statement for continue
                        break;

                    case 'grid_form':
                        if( !in_array( $hook, array( 'post.php', 'post-new.php' ) ) || $screen->post_type != WPUPG_EPOST_TYPE ) continue 2;
                        break;
            ...
            if( isset( $asset['setting'] ) && count( $asset['setting'] ) == 2 ) {
                if( WPUltimateEPostGrid::option( $asset['setting'][0], $asset['setting'][1] ) != $asset['setting'][1] ) continue;
            }

            // Check if setting does not equal value
            if( isset( $asset['setting_inverse'] ) && count( $asset['setting_inverse'] ) == 2 ) {
                if( WPUltimateEPostGrid::option( $asset['setting_inverse'][0], $asset['setting_inverse'][1] ) == $asset['setting_inverse'][1] ) continue;
            }
    ...
    private function enqueue_css( $assets )
    {
        $i = 1;
        foreach( $assets as $asset ) {
            $version = WPUltimateEPostGrid::option( 'assets_use_cache', '1' ) == '1' ? WPUPG_EVERSION : time();
