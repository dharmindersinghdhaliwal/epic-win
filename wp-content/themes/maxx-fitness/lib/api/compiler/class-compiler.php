 ...
 */
final class maxxfitness_tt_Compiler {
	...
	public function __construct( $args ) {
		...
		$this->dir = maxxfitness_get_compiler_dir( is_admin() ) . $this->compiler['id'];
		$this->url = maxxfitness_get_compiler_url( is_admin() ) . $this->compiler['id'];
	...
	public function set_fragments() {
		...
		if ( $added_fragments = maxxfitness_get( $this->compiler['id'], $maxxfitness_tt_compiler_added_fragments[$this->compiler['format']] ) )
		 ...
		 */
		$this->compiler['fragments'] = apply_filters( 'maxxfitness_compiler_fragments_' . $this->compiler['id'], $this->compiler['fragments'] );
	...
	public function set_filname() {
		...
		if ( !maxxfitness_tt_is_compiler_dev_mode() || !@is_dir( $this->dir ) )
		...
		foreach ( $this->compiler['fragments'] as $id => $fragment ) {
			...
			if ( strpos( $fragment, $_SERVER['HTTP_HOST'] ) !== false || preg_match( '#^\/[^\/]#', $fragment ) == true )
				$fragments_filemtime[$id] = @filemtime( maxxfitness_url_to_path( $fragment ) );
	...
	public function cache_file_exist() {
		...
		if ( ( $filname = maxxfitness_get( 'filename', $this->compiler ) ) && file_exists( $this->dir . '/' . $filname ) )
			return true;
	...
	public function get_url() {
		...
		$url = trailingslashit( $this->url ) . maxxfitness_get( 'filename', $this->compiler );
	...
	public function get_internal_content() {
		...
		if ( !file_exists( $fragment ) ) {
			...
			$fragment = maxxfitness_url_to_path( $fragment );
	...
	public function add_content_media_query( $content ) {
		...
		if ( !( $query = maxxfitness_get( 'query', $parse_url ) ) || stripos( $query, 'maxxfitness_compiler_media_query' ) === false )
			return $content;

		// Wrap the content in the query.
		$new_content = '@media ' . maxxfitness_get( 'maxxfitness_compiler_media_query', wp_parse_args( $query ) ) . ' {' . "\n";
	...
	public function format_content( $content ) {
		...
		if ( $this->compiler['type'] == 'style' ) {
			...
			if ( $this->compiler['format'] == 'less' ) {
				...
				if ( !class_exists( 'maxxfitness_Lessc' ) )
					require_once( maxxfitness_API_PATH . 'compiler/vendors/lessc.php' );

				$less = new maxxfitness_Lessc();

				$content = $less->compile( $content );

			}

			if ( !maxxfitness_tt_is_compiler_dev_mode() )
				$content = $this->strip_whitespace( $content );

		}

		if ( $this->compiler['type'] == 'script' && !maxxfitness_tt_is_compiler_dev_mode() && $this->compiler['minify_js'] ) {

			if ( !class_exists( 'JSMin' ) )
				require_once( maxxfitness_API_PATH . 'compiler/vendors/js-minifier.php' );
	...
	public function css_path_to_url( $matches, $base_is_path = false ) {
		...
		// Rebuild url and make sure it is a valid one using the maxxfitness_path_to_url function.
		$url = maxxfitness_path_to_url( trailingslashit( $base ) . $rebuilt_path );
	...
	public function kill() {
		...
		// Send report if set.
		if ( maxxfitness_get( 'maxxfitness_send_compiler_report' ) )
			$this->report();

		$html = maxxfitness_output( 'maxxfitness_compiler_error_title_text', sprintf(
			'<h2>%s</h2>',
			esc_html__( 'Not cool, Beans cannot work its magic :(', 'maxx-fitness' )
		) );

		$html .= maxxfitness_output( 'maxxfitness_compiler_error_message_text', sprintf(
			'<p>%s</p>',
			esc_html__( 'Your current install or file permission prevents Beans from working its magic. Please get in touch with Beans support, we will gladly get you started within 24 - 48 hours (working days).', 'maxx-fitness' )
		) );

		$html .= maxxfitness_output( 'maxxfitness_compiler_error_contact_text', sprintf(
			'<a class="button" href="http://www.getbeans.io/contact/?compiler_report=1" target="_blanc">%s</a>',
			esc_html__( 'Contact Beans Support', 'maxx-fitness' )
		) );

		$html .= maxxfitness_output( 'maxxfitness_compiler_error_report_text', sprintf(
			'<p style  =  "margin-top: 12px; font-size: 12px;"><a href="' . add_query_arg( 'maxxfitness_send_compiler_report', true ) . '">%1$s</a>. %2$s</p>',
			esc_html__( 'Send us an automatic report', 'maxx-fitness' ),
			esc_html__( 'We respect your time and understand you might not be able to contact us.', 'maxx-fitness' )
		) );
	...
	public function report() {
		...
		wp_die( maxxfitness_output( 'maxxfitness_compiler_report_error_text', sprintf(
			'<p>%s<p>',
			esc_html__( 'Thanks for your contribution by reporting this issue. We hope to hear from you again.', 'maxx-fitness' )
		) ) );
