...
class WPUPG_Epagination_Load_More extends WPUPG_Epremium_Addon {
    ...
    public function __construct( $name = 'pagination-load-more' ) {
        ...
        add_filter( 'wpupg_epagination_shortcode', array( $this, 'pagination_shortcode' ), 10, 2 );
    ...
    public function get_posts_args( $args, $page, $egrid ) {
        if( $egrid->pagination_type() == 'load_more' ) {
            $pagination_options = $egrid->pagination();
    ...
    public function pagination_shortcode( $pagination, $egrid ) {
        if( $egrid->pagination_type() == 'load_more' ) {
            ...
            if( $nbr_posts > $pagination_options['initial_posts'] ) {
                ...
                $pagination .= '<div id="wpupg-egrid-' . $egrid->slug() . '-pagination" class="wpupg-pagination wpupg-pagination-load_more" style="text-align: ' . $pagination_style['alignment'] . ';" data-grid="' . $egrid->slug() . '" data-type="load_more"' . $style_data . '>';
...
WPUltimateEPostGrid::loaded_addon( 'pagination-load-more', new WPUPG_Epagination_Load_More() );