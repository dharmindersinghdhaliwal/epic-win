 ...
 */
function maxxfitness_register_options( array $fields, $menu_slug, $section, $args = array() ) {
	 ...
	 */
	$fields = apply_filters( "maxxfitness_options_fields_{$section}", maxxfitness_tt_pre_standardize_fields( $fields ) );
	 ...
	 */
	$menu_slug = apply_filters( "maxxfitness_options_menu_slug_{$section}", $menu_slug );

	// Stop here if the page isn't concerned.
	if ( ( maxxfitness_get( 'page' ) !== $menu_slug ) || !is_admin() )
		return;

	// Stop here if the field can't be registered.
	if ( !maxxfitness_register_fields( $fields, 'option', $section ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
	require_once( maxxfitness_API_PATH . 'options/class.php' );

	$class = new maxxfitness_tt_Options();
 ...
 */
function maxxfitness_options( $menu_slug ) {

	if ( !class_exists( 'maxxfitness_tt_Options' ) )
		return false;
...
add_action( 'wp_loaded', 'maxxfitness_tt_options_page_actions' );
 ...
 * @ignore
 */
function maxxfitness_tt_options_page_actions() {

	if ( !maxxfitness_post( 'maxxfitness_options_nonce' ) )
		return;
