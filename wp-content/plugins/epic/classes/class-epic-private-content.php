...
class epic_Private_Content {
        ...
        foreach ($args as $sh_attr => $sh_value) {
        	switch ($sh_attr) {
	        	...
	        	case 'custom_restriction_key':
	        		/* epic Filters for applying custom rules on private content verification */
                    ...
                    $custom_restriction_type = isset($args['type']) ? $args['type'] : '';
                
	                $validate_private_content_params = array('user_id' => $this->current_user_id, 'value'=> $custom_restriction_value, 'key' => $custom_restriction_key, 'type' => $custom_restriction_type );
	                
	                $this->status = apply_filters( 'epic_validate_private_content', $this->status , $validate_private_content_params);
	                // End Filters
	...
	public function allowed_roles_filter($roles){
		...
		$user_roles = $epic_roles->epic_get_user_roles_by_id($this->current_user_id);
		$roles = explode(',', $roles);
        ...
        if (!$private_content_result['status']) {
            ...
            if ($message !== 'off') {
            	...
            	switch ($private_content_result['type']) {
            		case 'guest':
            			/* filter wildcards */
		                $html = $epic->get_option('html_private_content');
		                $html = str_replace("{epic_current_uri}", $epic->current_page, $html);
		                $display .= wpautop($html);

		                if($epic->get_option('html_private_content_form')){
            		...
            		case 'allowed_meta_key':
            		case 'blocked_meta_key':
		        		$html = $epic->get_option('html_members_private_content');
		                $html = str_replace("{epic_current_uri}", $epic->current_page, $html);
		                $display .= wpautop($html);

		                $params = array('restriction_type' => $private_content_result['type']);
		                $display = apply_filters('epic_private_content_restriction_info',$display,$params);
		        		break;
		        	default:
		        		$html = $epic->get_option('html_members_private_content');
		                $html = str_replace("{epic_current_uri}", $epic->current_page, $html);
...
$epic_private_content =  new epic_Private_Content();
