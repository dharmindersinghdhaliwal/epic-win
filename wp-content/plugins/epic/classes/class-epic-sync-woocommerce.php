...
class epic_Sync_Woo {
	...
	function __construct() {
		...
		if('admin.php' == $pagenow && isset($_GET['page']) && ('epic-sync-tools' == $_GET['page']) 
			...
			if ( isset($_REQUEST['sync']) && !isset($_POST['submit']) && !isset($_POST['epic-add']) && !isset($_POST['reset-options']) && !isset($_POST['reset-options-fields']) ) {
				...
				if(file_exists($this->woocommerce_file_path)){
					...
					echo __('Woocommerce File Doesn\'t Exist','epic');exit;
				}	
				...
				echo '<div class="updated"><p><strong>'.__('WooCommerce customer fields have been added successfully.','epic').'</strong></p></div>';
...
add_action( 'init', 'epic_woocommerce_plugin_init' );
function epic_woocommerce_plugin_init(){
    global $epic_sync_woocommerce;
    if(class_exists('WooCommerce')){
        $epic_sync_woocommerce = new epic_Sync_Woo();
    }
}