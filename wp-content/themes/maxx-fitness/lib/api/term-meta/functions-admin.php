 ...
 */
function maxxfitness_register_term_meta( array $fields, $taxonomies, $section ) {
	 ...
	 */
	$fields = apply_filters( "maxxfitness_term_meta_fields_{$section}", maxxfitness_tt_pre_standardize_fields( $fields ) );
	 ...
	 */
	$taxonomies = apply_filters( "maxxfitness_term_meta_taxonomies_{$section}", (array) $taxonomies );

	// Stop here if the current page isn't concerned.
	if ( !maxxfitness_tt_is_admin_term( $taxonomies ) || !is_admin() )
		return;

	// Stop here if the field can't be registered.
	if ( !maxxfitness_register_fields( $fields, 'term_meta', $section ) )
		return false;

	// Load the class only if this function is called to prevent unnecessary memory usage.
	require_once( maxxfitness_API_PATH . 'term-meta/class.php' );

	new maxxfitness_tt_Term_Meta( $section );
 ...
 * @ignore
 */
function maxxfitness_tt_is_admin_term( $taxonomies ) {

	if ( !$taxonomy = maxxfitness_get_or_post( 'taxonomy' ) )
		return false;
