...
class WPUPG_Parser {
    ...
    public function __construct( $blocks )
    {
        ...
        $this->template = new WPUPG_Etemplate( $this->template_blocks, $this->fonts );
    }
    ...
    {
        return new WPUPG_Etemplate_Container();
    }
    ...
    protected function parseBlockRows( $block )
    {
        ...
        $template_rows = new WPUPG_Etemplate_Rows();
    ...
    protected function parseBlockColumns( $block )
    {
        ...
        $template_columns = new WPUPG_Etemplate_Columns ();
    ...
    {
        $template_date = new WPUPG_Etemplate_Date ();
    ...
    {
        $image = new WPUPG_Etemplate_Image();
    ...
    {
        $template_code = new WPUPG_Etemplate_Code();
    ...
    {
        $image = new WPUPG_Etemplate_Post_Image();
    ...
    {
        $template_date = new WPUPG_Etemplate_Post_Date();
        $format = isset( $block->dateFormat ) ? $block->dateFormat : '';
        return $template_date->format($format);
    }

    protected function parseBlockPostTitle( $block )    { return new WPUPG_Etemplate_Post_Title(); }
    protected function parseBlockPostContent( $block )  { return new WPUPG_Etemplate_Post_Content(); }
    protected function parseBlockPostExcerpt( $block )  { return new WPUPG_Etemplate_Post_Excerpt(); }
    protected function parseBlockPostAuthor( $block )   { return new WPUPG_Etemplate_Post_Author(); }

    protected function parseBlockPostTaxonomyTerms( $block )   {
        $terms = new WPUPG_Etemplate_Post_Taxonomy_Terms();
    ...
    {
        $egrid_id = WPUltimateEPostGrid::option('template_editor_preview_grid');
        ...
        if( $egrid_id && WPUPG_EPOST_TYPE == get_post_type( $egrid_id ) ) {
            $src = get_permalink( $egrid_id );
            if( strpos( $src, '?' ) ) {
                $src = $src . '&wpupg_etemplate_editor_preview=' . $egrid_id;
            } else {
                $src = $src . '?wpupg_etemplate_editor_preview=' . $egrid_id;
            }
            ...
            _e( 'Please set a Preview Egrid in the settings first.', 'wp-eultimate-post-grid' );
            echo '</div>';
        }
    ...
    {
        if( $save_as_new ) {
            $new_id = WPUltimateEPostGrid::addon( 'custom-templates' )->add_template( $new_name, $this->template );
            ...
            WPUltimateEPostGrid::addon( 'custom-templates' )->update_template( $id, $this->template );
