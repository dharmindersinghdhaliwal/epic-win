 ...
 * @package API\Uikit
 */
final class maxxfitness_tt_Uikit {
	...
	function compile() {
		 ...
		 */
		$styles = apply_filters( 'maxxfitness_uikit_euqueued_styles', $this->register_less_components() );
		 ...
		 */
		$scripts = apply_filters( 'maxxfitness_uikit_euqueued_scripts', $this->register_js_components() );
		 ...
		 */
		$styles_args = apply_filters( 'maxxfitness_uikit_euqueued_styles_args', array() );
		 ...
		 */
		$scripts_args = apply_filters( 'maxxfitness_uikit_euqueued_scripts_args', array(
			'depedencies' => array( 'jquery' )
		) );

		// Compile less.
		if ( $styles )
			maxxfitness_compile_less_fragments( 'uikit', array_unique( $styles ), $styles_args );

		// Compile js.
		if ( $scripts )
			maxxfitness_compile_js_fragments( 'uikit', array_unique( $scripts ), $scripts_args );
	...
	function register_less_components() {
		...
		foreach ( $maxxfitness_tt_uikit_enqueued_items['components'] as $type => $items ) {
		...
		// Add fixes.
		if ( !empty( $components ) )
			$components = array_merge( $components, array( maxxfitness_API_PATH . 'uikit/src/fixes.less' ) );
	...
	function get_less_directories( $type ) {
		...
		$directories = array( maxxfitness_API_PATH . 'uikit/src/less/' . $type );
		// Add the registered theme directories.
		foreach ( $maxxfitness_tt_uikit_enqueued_items['themes'] as $id => $directory )
	...
	function get_js_directories( $type ) {
		...
		// Define uikit src directory.
		return array( maxxfitness_API_PATH . 'uikit/src/js/' . $type );
	...
	function get_autoload_components( $components ) {
		...
		foreach ( (array) $components as $component ) {
			...
			$this_depedencies = maxxfitness_get( $component, $depedencies, array() );

			foreach ( $this_depedencies as $depedency ) {
				$autoload['core'] = array_merge( $autoload['core'], array_flip( maxxfitness_get( 'core', $this_depedencies, array() ) ) );
				$autoload['add-ons'] = array_merge( $autoload['add-ons'], array_flip( maxxfitness_get( 'add-ons', $this_depedencies, array() ) ) );
			}
