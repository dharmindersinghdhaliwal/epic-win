<?php
/*
  Plugin Name: MaxxFitness Trainers widget
  Plugin URI: http://themeforest.net/user/torbara/?ref=torbara
  Description: MaxxFitness Trainers widget
  Author: Torbara
  Version: 3.0.0
  Author URI: http://themeforest.net/user/torbara/portfolio/?ref=torbara
 */

class MaxxFitnessTrainers extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'MaxxFitnessTrainers', 'description' => 'Displays a Features posts with thumbnail');
        parent::__construct('MaxxFitnessTrainers', 'MaxxFitness Trainers', $widget_ops);
    }

    function form($instance) { ?>

        <?php $title = isset( $instance['title']) ? esc_attr( $instance['title'] ) : "MaxxFitness Trainers"; ?>
        <p>
...
//add_action('widgets_init', create_function('', 'return register_widget("MaxxFitnessTrainers");'));

add_action('widgets_init', 'MaxxFitnessTrainers');

function MaxxFitnessTrainers()
{
    return register_widget('MaxxFitnessTrainers');
}
