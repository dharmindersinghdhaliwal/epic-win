...
WPUltimateEPostEgrid.grids = {};

WPUltimateEPostEgrid.initEgrid = function(container) {
    ...
    WPUltimateEPostEgrid.grids[grid_id] = {
        container: container,
    ...
    WPUltimateEPostEgrid.checkLinks(grid_id);
};

WPUltimateEPostEgrid.filterEgrid = function(grid_id) {
    var grid = WPUltimateEPostEgrid.grids[grid_id];
    var filters = [];
    ...
    grid.filter = '';
    WPUltimateEPostEgrid.getFilterString(grid_id, '', filters, 0);

    grid.container.isotope({ filter: grid.filter });
    grid.container.trigger('wpupgFiltered');
    WPUltimateEPostEgrid.updateDeeplink();
};

WPUltimateEPostEgrid.getFilterString = function(grid_id, s, attrs, k) {
    if(k==attrs.length) {
        if(WPUltimateEPostEgrid.grids[grid_id].filter !== '') s = ', ' + s;
        WPUltimateEPostEgrid.grids[grid_id].filter += s;
    } else {
        for(var i=0; i<attrs[k].length;i++) {
            WPUltimateEPostEgrid.getFilterString(grid_id, s+attrs[k][i], attrs, k+1);
        }
    }
};

WPUltimateEPostEgrid.updateDeeplink = function() {
    var link = '';

    for(var grid_id in WPUltimateEPostEgrid.grids) {
        if(WPUltimateEPostEgrid.grids.hasOwnProperty(grid_id)) {
            var grid = WPUltimateEPostEgrid.grids[grid_id];
            var grid_link = '';
...
WPUltimateEPostEgrid.restoreDeeplink = function() {
        ...
        for(var i=0; i < grids.length; i++) {
            ...
            for(var j=1; j < parts.length; j++) {
                ...
                if(filter[0] == 'p') {
                    ...
                    WPUltimateEPostEgrid.updatePagination(jQuery('#wpupg-grid-' + grid_id + '-pagination'), page);
                } else {
                    var terms = filter[1].split(',');
                    WPUltimateEPostEgrid.updateFilter(jQuery('#wpupg-grid-' + grid_id + '-filter'), filter[0], terms);
                }
            }
        }
    }
...
WPUltimateEPostEgrid.checkLinks = function(grid_id) {
    var grid = WPUltimateEPostEgrid.grids[grid_id];
    ...
    if(link_type != 'none') {
        grid.container.find('.wpupg-item').each(function() {
            ...
            item.find('a:not(.wpupg-keep-link, .wpurp-exercise-favorite, .wpurp-exercise-add-to-shopping-list, .wpurp-exercise-print-button, .wpurp-exercise-grid-link)').each(function() {
...
WPUltimateEPostEgrid.updateFilter = function(container, taxonomy, terms) {
    var type = container.data('type');
    var func = 'updateFilter' + type.charAt(0).toUpperCase() + type.slice(1);
    if(typeof WPUltimateEPostEgrid[func] == 'function') {
        WPUltimateEPostEgrid[func](container, taxonomy, terms);
    }
};

WPUltimateEPostEgrid.updateFilterIsotope = function(container, taxonomy, terms) {
    for(var i = 0; i < terms.length; i++) {
        container.find('.wpupg-filter-tag-' + terms[i]).click();
    }
};

WPUltimateEPostEgrid.initFilterIsotope = function(container) {
    var grid_id = container.data('grid');

    WPUltimateEPostEgrid.grids[grid_id].multiselect = container.data('multiselect');
    ...
    container.find('.wpupg-filter-isotope-term').click(function() {
        ...
        WPUltimateEPostEgrid.grids[grid_id].filters = filters;
...
WPUltimateEPostEgrid.updatePagination = function(container, page) {
    var type = container.data('type');
    var func = 'updatePagination' + type.charAt(0).toUpperCase() + type.slice(1);
    if(typeof WPUltimateEPostEgrid[func] == 'function') {
        WPUltimateEPostEgrid[func](container, page);
    }
};

WPUltimateEPostEgrid.updatePaginationPages = function(container, page) {
    container.find('.wpupg-page-' + page).click();
};

WPUltimateEPostEgrid.initPaginationPages = function(container) {
    var grid_id = container.data('grid');
    ...
    container.find('.wpupg-pagination-term').click(function() {
        ...
        if(WPUltimateEPostEgrid.grids[grid_id].pages.indexOf(page) !== -1) {
            WPUltimateEPostEgrid.grids[grid_id].page = page;
            WPUltimateEPostEgrid.filterEgrid(grid_id);
        } else {
            ...
            pagination_item.css('color', WPUltimateEPostEgrid.grids[grid_id].pagination_style.active_background_color);

            // Get exercises through AJAX
            jQuery.post(wpupg_public.ajax_url, data, function(html) {
                var posts = jQuery(html).toArray();
                WPUltimateEPostEgrid.grids[grid_id].container.isotope('insert', posts);
                WPUltimateEPostEgrid.grids[grid_id].page = page;
                WPUltimateEPostEgrid.filterEgrid(grid_id);
                WPUltimateEPostEgrid.checkLinks(grid_id);

                WPUltimateEPostEgrid.grids[grid_id].container.imagesLoaded( function() {
                    WPUltimateEPostEgrid.grids[grid_id].container.isotope('layout');
                });

                pagination_item.removeClass('wpupg-spinner');
                pagination_item.css('color', WPUltimateEPostEgrid.grids[grid_id].pagination_style.active_text_color);

                WPUltimateEPostEgrid.grids[grid_id].pages.push(page);
            });
        }
    });
...
jQuery(document).ready(function($) {
    $('.wpupg-grid').each(function() {
        WPUltimateEPostEgrid.initEgrid($(this));
    });
    ...
    $('.wpupg-filter').each(function() {
        ...
        WPUltimateEPostEgrid.grids[grid_id].filter_type = type;

        var func = 'initFilter' + type.charAt(0).toUpperCase() + type.slice(1);
        if(typeof WPUltimateEPostEgrid[func] == 'function') {
            WPUltimateEPostEgrid[func](container);
        }
    });
    ...
    $('.wpupg-pagination').each(function() {
        ...
        WPUltimateEPostEgrid.grids[grid_id].pagination_type = type;
    ...
    WPUltimateEPostEgrid.restoreDeeplink();
});