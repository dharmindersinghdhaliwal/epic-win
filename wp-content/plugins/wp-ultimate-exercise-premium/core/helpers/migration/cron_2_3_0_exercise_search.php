...
if( $query->have_posts() ) {
    ...
    foreach( $posts as $post ) {
        ...
        $post_content = preg_replace("/<div class=\"wpuep-searchable-exercise\"[^<]*<\/div>/", "", $post->post_content);
        $post_content .= '<div class="wpuep-searchable-exercise" style="display:none">';
        ...
        update_post_meta( $exercise->ID(), 'wpuep_text_search', time() );
    }
} else {
    // Finished migrating, all exercises have a full text search
    update_option( 'wpuep_cron_migrate_version', '2.3.0' );
}