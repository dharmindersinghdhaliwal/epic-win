    ...
    <tr id="wpupg_rule_0" data-rule="0">
        <td>
            <select name="wpupg_limit_posts_rule[0][field]" id="wpupg_limit_posts_rule_field_0" class="wpupg_rule_field">
                <optgroup label="<?php _e( 'General', 'wp-eultimate-post-grid' ); ?>">
                    <?php
                    $rule_field_options = array(
                        'wpupg_general|author' => __( 'Author', 'wp-eultimate-post-grid' ),
                        'wpupg_general|date' => __( 'Date', 'wp-eultimate-post-grid' ),
                        'wpupg_general|id' => __( 'Post ID', 'wp-eultimate-post-grid' ),
                    );

                    foreach( $rule_field_options as $rule_field => $rule_field_name ) {
                        $selected = '';//$rule_field == $egrid->filter_type() ? ' selected="selected"' : '';
                ...
                foreach( $post_types as $post_type => $options ) {
                    ...
                    if( count( $taxonomies ) > 0 ) {
                        echo '<optgroup label="' . __( 'Taxonomies', 'wp-eultimate-post-grid' ) . ' (' . $options->labels->name . ')">';
        ...
        <td>
            <div class="rule_container rule_container_wpupg_general_author">
                <select name="wpupg_limit_posts_rule[0][values_wpupg_general_author][]" id="wpupg_limit_posts_rule_values_wpupg_general_author_0" multiple>
                    ...
                    foreach( $authors as $author ) {
                        $selected = '';//$rule_type == $egrid->filter_type() ? ' selected="selected"' : '';
            ...
            <div class="rule_container rule_container_wpupg_general_date">
                <select name="wpupg_limit_posts_rule[0][values_wpupg_general_date_condition]" id="wpupg_limit_posts_rule_values_wpupg_general_date_condition_0">
                    ...
                    $date_condition_options = array(
                        'before' => __( 'Before', 'wp-eultimate-post-grid' ),
                        'is' => __( 'Is', 'wp-eultimate-post-grid' ),
                        'after' => __( 'After', 'wp-eultimate-post-grid' ),
                    );
            ...
            <div class="rule_container rule_container_wpupg_general_id">
                <input type="text" name="wpupg_limit_posts_rule[0][values_wpupg_general_id]" id="wpupg_limit_posts_rule_values_wpupg_general_id_0" placeholder="<?php _e( 'Separate post IDs with a semicolon', 'wp-eultimate-post-grid' ); ?> (1;28;88)"/>
            </div>
            <?php
        ...
        <td>
            <select name="wpupg_limit_posts_rule[0][type]" id="wpupg_limit_posts_rule_type_0">
                ...
                $rule_type_options = array(
                    'restrict' => __( 'Only these posts', 'wp-eultimate-post-grid' ),
                    'exclude' => __( 'Exclude these posts', 'wp-eultimate-post-grid' ),
                );
        ...
        <td>
            <img src="<?php echo WPUltimateEPostGrid::get()->coreUrl . '/img/trash.png'; ?>" class="wpupg_rule_delete" title="<?php _e( 'Delete Rule', 'wp-eultimate-post-grid' ); ?>"/>
        </td>
    </tr>
    </tbody>
    <tbody id="wpupg_rules">
    <?php
    $limit_rules = $egrid->limit_rules();
        ...
        <tr id="wpupg_rule_<?php echo $rule_id; ?>" data-rule="<?php echo $rule_id; ?>">
            ...
            <td>
                ...
                <div class="rule_container rule_container_wpupg_general_id">
                    ...
                    <input type="text" name="wpupg_limit_posts_rule[<?php echo $rule_id; ?>][values_wpupg_general_id]" id="wpupg_limit_posts_rule_values_wpupg_general_id_<?php echo $rule_id; ?>" value="<?php echo $id_value; ?>" placeholder="<?php _e( 'Separate post IDs with a semicolon', 'wp-eultimate-post-grid' ); ?> (1;28;88)"/>
                </div>
                <?php
                foreach( $rule_fields as $rule_field ) {
                    $rule_field_parts = explode( '|', $rule_field );
...
<a href="#" id="wpupg_add_rule"><?php _e( 'Add Rule', 'wp-eultimate-post-grid' ); ?></a>