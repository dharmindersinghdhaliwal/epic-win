...
// Set default panel
if ($panel == '' && in_array($widget->position, array('top-a', 'top-b', 'top-c', 'top-d', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b', 'footer'))) {
...
// Set title
if (in_array($widget->position, array('toolbar-r' ,'toolbar-l'))) {
	$title = '';
} elseif ($title && !($widget->position == 'menu')) {
        
        $title = '<h3 class="uk-panel-title">'.$icon.$title.'</h3>';
	//$title = '<div class="ak-icon">'.$icon.'</div><h3 class="uk-panel-title">'.$title.'</h3>';
}
...
// Render widget
if (in_array($widget->position, array('breadcrumbs', 'logo', 'social', 'search', 'fullscreen', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
	echo $content;
