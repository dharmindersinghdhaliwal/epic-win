...
class WPUPG_Elimit_Posts extends WPUPG_Epremium_Addon {

    public function __construct( $name = 'limit-posts' ) {
        parent::__construct( $name );

        add_filter( 'wpupg_egrid_cache_post_ids', array( $this, 'grid_cache_post_ids'), 10, 2 );
    }

    public function grid_cache_post_ids( $post_ids, $egrid ) {
        if( $egrid->limit_posts() ) {
            $rules = $egrid->limit_rules();
...
WPUltimateEPostGrid::loaded_addon( 'limit-posts', new WPUPG_Elimit_Posts() );