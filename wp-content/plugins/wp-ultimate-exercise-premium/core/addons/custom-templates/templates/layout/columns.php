...
class WPUEP_Template_Columns extends WPUEP_Template_Block {
...
<div class="wpuep-responsive-mobile">
    <div<?php echo $this->style(); ?>>
        <?php if( $this->mobile_reverse ) { ?>
            <?php for( $j = $this->columns-1; $j >= 0; $j-- ) { ?>
                <?php if( $this->show( $exercise, 'col-' . $j, $args ) ) { ?>
                    <div class="wpuep-rows-row">
                        <?php $this->output_children( $exercise, 0, $j, $args ); ?>
                    </div>
                <?php } // end if show col ?>
            <?php } // end for cols ?>
        <?php } else { ?>
...
<div class="wpuep-responsive-desktop">
<?php } ?>
