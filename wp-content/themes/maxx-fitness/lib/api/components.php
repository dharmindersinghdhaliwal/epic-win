 ...
 */
function maxxfitness_load_api_components( $components ) {

	static $loaded = array();

	$root = maxxfitness_API_PATH;
	...
	foreach ( (array) $components as $component ) {
		...
		if ( array_key_exists( $component, $dependencies ) )
			maxxfitness_load_api_components( $dependencies[$component] );
		 ...
		 * @since 1.0.0
		 */
		do_action( 'maxxfitness_loaded_api_component_' . $component );
 ...
 * @return bool Will always return true.
 */
function maxxfitness_add_api_component_support( $feature ) {

	global $maxxfitness_tt_api_components_support;
	...
	$maxxfitness_tt_api_components_support[$feature] = $args;
 ...
 */
function maxxfitness_get_component_support( $feature ) {

	global $maxxfitness_tt_api_components_support;

	if ( !isset( $maxxfitness_tt_api_components_support[$feature] ) )
		return false;

	return $maxxfitness_tt_api_components_support[$feature];
 ...
 * @return bool Will always return true.
 */
function maxxfitness_remove_api_component_support( $feature ) {

	global $maxxfitness_tt_api_components_support;

	unset( $maxxfitness_tt_api_components_support[$feature] );
...
if ( !isset( $maxxfitness_tt_api_components_support ) )
	$maxxfitness_tt_api_components_support = array();