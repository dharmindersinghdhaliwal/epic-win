...
if( stripos( __FILE__, 'vendor/wp-eultimate-post-grid-premium' ) ) {
...
class WPUPG_Parser {
    ...
    {
        $grid_id = WPUltimateEPostEgrid::option('template_editor_preview_grid');
        ...
        if( $grid_id && WPUPG_EPOST_TYPE == get_post_type( $grid_id ) ) {
    ...
    {
        if( $save_as_new ) {
            $new_id = WPUltimateEPostEgrid::addon( 'custom-templates' )->add_template( $new_name, $this->template );
            ...
            WPUltimateEPostEgrid::addon( 'custom-templates' )->update_template( $id, $this->template );
