...
class WPUEP_Template_Block {
    ...
    public function output_children_string( $exercise, $row = 0, $column = 0, $args = array() )
    {
        $output = '';
        ...
        if( isset( $this->children[$row][$column] ) ) {
            ...
            {
                $output .= $child->output( $exercise, $args );
            }
        }
    ...
    public function output_children( $exercise, $row = 0, $column = 0, $args = array() )
    {
        echo $this->output_children_string( $exercise, $row, $column, $args );
    }
    ...
    protected function style( $names = 'default' )
    {
        ...
        if( WPUltimateExercise::option( 'exercise_template_inline_css', '1' ) == '1' ) {
        ...
        if( in_array( $this->type, array( 'exercise-ingredient-container', 'exercise-instruction-container' ) ) ) {
    ...
    private function condition( $exercise, $condition, $args = array() )
    {
        $show = true;

        if( $condition['condition_type'] == 'field' ) {
            $present = $exercise->is_present( $condition['field'] );
            ...
            if( $condition['setting'] == 'user_menus_add_to_shopping_list' ) {
                if( !WPUltimateExercise::is_premium_active() ) {
                    ...
                    $setting = WPUltimateExercise::option( 'user_menus_add_to_shopping_list', 'off' );
            ...
            } else if( $condition['setting'] == 'favorite_exercises_enabled' ) {
                if( !WPUltimateExercise::is_premium_active() ) {
                    $val = false;
                } else {
                    $val = WPUltimateExercise::option( 'favorite_exercises_enabled', '0' );
                }
                ...
                // Default 0
                $val = WPUltimateExercise::option( $condition['setting'], '0' );
            } else {
                // Default 1
                $val = WPUltimateExercise::option( $condition['setting'], '1' );
            }

            if( $condition['setting'] == 'exercise_adjustable_units' && !WPUltimateExercise::is_premium_active() ) {
            ...
            if( has_term( $condition['term'], $condition['taxonomy'], $exercise->ID() ) ) {
    ...
    protected function show( $exercise, $target = 'block', $args = array() )
    {
        if( isset( $this->conditions[$target] ) ) {
            foreach( $this->conditions[$target] as $condition ) {
                if( !$this->condition( $exercise, $condition, $args ) ) {
                    return false;
                }
            }
        }
    ...
    */
     ...
     */
    protected function output_block( $exercise, $args )
    {
        return $this->show( $exercise, 'block', $args );
    }
    ...
    protected function before_output()
    {
        ...
        if( !$this->show_on_desktop ) {
            $output = '<div class="wpuep-responsive-mobile">';
        } else if( !$this->show_on_mobile ) {
            $output = '<div class="wpuep-responsive-desktop">';
        }
        ...
        if( $this->background_preset )
        {
            switch( $this->background_preset ) {
                case 'default':
                    $img = WPUltimateExercise::addon( 'custom-templates' )->addonUrl . '/img/default.png';
                    break;
                default:
                    $img = WPUltimateExercise::addon( 'template-editor' )->addonUrl . '/img/' . $this->background_preset . '.png';
            }
    ...
    protected function after_output( $output, $exercise )
    {
        ...
        if( $this->link_color && WPUltimateExercise::option( 'exercise_template_inline_css', '1' ) == '1' ) {
        ...
        return apply_filters( 'wpuep_output_exercise_block_' . $this->type, $output, $exercise, $this );
    }
