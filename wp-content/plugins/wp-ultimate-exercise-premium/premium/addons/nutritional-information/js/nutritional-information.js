...
// Variables
NutritionalInformation.loader = '<div class="loader"><div></div><div></div><div></div></div>';
NutritionalInformation.nutritional_results = [];
// Functions
NutritionalInformation.init = function() {
    // Get Nutritional Data button
    jQuery('.wpuep-get-nutritional-panel').on('click', function() {
        ...
        NutritionalInformation.closePanel(ingredient, function() {
            ...
            NutritionalInformation.searchIngredientExercise(ingredient);
        });
    });
    // Edit Nutritional Data button
    jQuery('.wpuep-edit-nutritional-panel').on('click', function() {
        var ingredient = jQuery(this).data('ingredient');
        NutritionalInformation.closePanel(ingredient, function() {
    ...
    // Limit ingredients shown by exercise
    jQuery('#limit-exercise').select2({
        width: 'element'
    }).on('change', function() {
        window.location = window.location.href + '&limit_by_exercise=' + jQuery(this).val();
    });
}
...
NutritionalInformation.searchIngredientExercise = function(ingredient) {
    ...
    if( name.length > 0 ) {
        ...
        results.slideUp(500);
        var data = {
            action: 'search_ingredients_exercise',
            security: wpuep_nutritional_information.nonce,
            name: name
        };
        jQuery.post(wpuep_nutritional_information.ajaxurl, data, function(ingredients) {
            loader.remove();
            results.empty();
...
NutritionalInformation.getNutritional = function(ingredient, food_id) {
    ...
    results.slideUp(500);
    var data = {
        action: 'get_nutritional_exercise',
        security: wpuep_nutritional_information.nonce,
        food: food_id
    };
    jQuery.post(wpuep_nutritional_information.ajaxurl, data, function(nutritional) {
        loader.remove();
        results.empty();
...
NutritionalInformation.saveNutritional = function(ingredient) {
    ...
    jQuery.post(wpuep_nutritional_information.ajaxurl, data, function(html) {
        loader.remove();
        summary.html(html);
    }, 'html');
}

NutritionalInformation.calculateExercise = function(exercise) {
    var system = ExerciseUnitConversion.determineIngredientListSystem(jQuery('.calculate-exercise-panel'));
    jQuery('.calculate-exercise-panel tr.ingredient-nutritional-match').each(function() {
        var row = jQuery(this);
        ...
        if(isNaN(base_amount)) {
            ...
            var base_unit = ExerciseUnitConversion.getUnitFromAlias(base_alias);
            // Output
            var converted = '';
            // Calculate converted amount
            if(unit !== undefined && amount) {
                var converted = NutritionalInformation.getConvertedAmount(system, amount, unit, base_unit);
            }
            // Set calculated amount
            var input = '= <input type="text" value="' + converted + '" data-base="'+base_amount+'"/> ' + base_alias;
            var summary = jQuery('#nutritional-summary-' +ingredient).html();
            row.find('td:nth-child(2)').html(input);
            row.find('td:nth-child(3)').html(summary);
        }
    });
    jQuery('.calculate-exercise-panel').slideUp(250, function() {
        jQuery('.calculate-exercise-panel').slideDown(500);
    });
}
...
NutritionalInformation.getConvertedAmount = function(old_system, amount, unit, new_unit) {
    ...
    var old_unit_type = ExerciseUnitConversion.getUnitType(unit);
    var new_unit_type = ExerciseUnitConversion.getUnitType(new_unit);
    ...
    // Calculate converted amount
    try {
        var new_amount = quantity.to(ExerciseUnitConversion.getAbbreviation(new_unit)).scalar;
    } catch (err) {
        var new_amount = '';
    }
    return new_amount;
...
NutritionalInformation.useCalculatedExercise = function(exercise) {
    var ingredients = [];
    var servings = parseInt(jQuery('.calculate-exercise-panel').data('servings'));
    jQuery('.calculate-exercise-panel tr.ingredient-nutritional-match').each(function() {
        var row = jQuery(this);
        var ingredient = row.data('ingredient');
    ...
    jQuery('.wpuep-exercise-nutritional .nutritional-data input').each(function() {
        var input = jQuery(this);
        var total = 0;
        ...
        input.val(ExerciseUnitConversion.formatNumber(total, false, 0));
    });
}

NutritionalInformation.saveExercise = function(exercise) {
    var loader = jQuery(NutritionalInformation.loader);
    jQuery('.save-exercise-buttons').append(loader);
    var nutritional = {};
    jQuery('.wpuep-exercise-nutritional .nutritional-data input').each(function() {
        var input = jQuery(this);
        var name = input.attr('name');
    ...
    var data = {
        action: 'save_nutritional_exercise',
        security: wpuep_nutritional_information.nonce,
        exercise: exercise,
        nutritional: nutritional
    };
...
NutritionalInformation.resetExercise = function(exercise) {
    jQuery('.wpuep-exercise-nutritional .nutritional-data input').each(function() {
        jQuery(this).val('');
    });
}