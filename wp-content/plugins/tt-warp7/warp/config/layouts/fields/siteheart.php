...
Torbara Support Team provides you with professional assistance in case you have some issues using our products or you need some advises in your work. Feel free to contact us through our Support Chat or by email <a href="mailto:info@torbara.com">info@torbara.com</a>. Please be aware we are working to the best of our abilities in order to reply to your request in earliest convenience, but due to busy workflow it might take 24-72 business hours to get reply from us.<br>
...
_shcp =[{widget_id : widget_id, w_type: 'embed', w_selector: '#tm-siteheart-box', side: 'top', position: 'left', callback : function(chat){ 
    ...
    setInterval(function (){
        ...
        if(jQuery(".sh_chat_logo").length){
            jQuery(".sh_chat_logo").after("<img src='<?php echo WP_PLUGIN_URL; ?>/tt_warp7/warp/config/images/ChatLogo.png' style='margin: 8px;float:left' />");
            jQuery(".sh_chat_logo").remove();
        }
        
        if(jQuery(".sh_logo_img").length){
            jQuery(".sh_logo_img").after("<img src='<?php echo WP_PLUGIN_URL; ?>/tt_warp7/warp/config/images/ChatLogo.png' style='margin: 6px;float:left' />");
            jQuery(".sh_logo_img").remove();
        }
    }, 200);
    // Hide copyrights (sorry)
    //    if(jQuery(".sh_copyright").length){
    //        jQuery(".sh_copyright").remove();
    //    }    
} }];
...
<iframe src="http://torbara.com/support-tab-iframe/wordpress.html" width="100%" height="800px" id="support-tab-iframe" marginheight="0" frameborder="0"></iframe>