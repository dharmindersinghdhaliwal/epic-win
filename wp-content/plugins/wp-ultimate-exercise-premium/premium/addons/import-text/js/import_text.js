...
jQuery(document).ready(function() {
    ...
    // Import exercises from other plugins
    if( wpuep_import_ingredients.length > 0 ) {
        for(var i = 0, l = wpuep_import_ingredients.length; i<l; i++) {
            var ingredient = ImportText.parseIngredient(wpuep_import_ingredients[i]);
            ingredient.group = '';
...
ImportText.init = function() {
    ...
    jQuery('#input-exercise').on('keyup change', function() {
    ...
    jQuery('.regions-button').on('click', function() {
        ...
        ImportText.exercise[type] = text;

        // Update corresponding form field and trigger change event
        if(type == 'instructions' || type == 'ingredients') {
            jQuery('#raw_exercise_'+type).val(text).change();
        } else {
            jQuery('#exercise_'+type).val(text).change();
        }
    });

    // Adaptive textareas
    jQuery('#raw_exercise_ingredients, #raw_exercise_instructions').on('keyup change', function() {
        ImportText.adaptiveTextarea(jQuery(this));
        ImportText.getSeparateLines(jQuery(this));
    });

    jQuery('#exercise_instructions_separate_type').on('change', function() {
        // Trigger change in instructions
        jQuery('#raw_exercise_instructions').change();
    });
};
...
ImportText.getSeparateLines = function(textarea) {
    ...
    if(id == 'raw_exercise_instructions') {
        type = jQuery('#exercise_instructions_separate_type option:selected').val();
    }
    ...
    target.empty();
    if(id == 'raw_exercise_instructions') {
        jQuery('#exercise_instructions_output').empty();
    } else {
...
ImportText.parseIngredient = function(text) {
    ...
    // Unit
    ingredient.unit = '';
    var units = wpuep_import_text.units;
...
ImportText.addIngredientRow = function(ingredient, i) {
    ...
    var amount = jQuery('<td><input type="text" name="exercise_ingredients['+i+'][amount]" class="ingredients_amount" id="ingredients_amount_'+i+'" value="'+ImportText.escapeAttr(ingredient.amount)+'" /></td>');
    var unit = jQuery('<td><input type="text" name="exercise_ingredients['+i+'][unit]" class="ingredients_unit" id="ingredients_unit_'+i+'" value="'+ImportText.escapeAttr(ingredient.unit)+'" /></td>');
    var name = jQuery('<td><input type="text" name="exercise_ingredients['+i+'][ingredient]" class="ingredients_name" id="ingredients_'+i+'" value="'+ImportText.escapeAttr(ingredient.name)+'" /></td>');
    var notes = jQuery('<td><input type="text" name="exercise_ingredients['+i+'][notes]" class="ingredients_notes" id="ingredients_notes_'+i+'" value="'+ImportText.escapeAttr(ingredient.notes)+'" /></td><input type="hidden" name="exercise_ingredients['+i+'][group]" class="ingredients_group" id="ingredient_group_'+i+'" value="'+ImportText.escapeAttr(ingredient.group)+'">');
...
ImportText.addInstructionRow = function(instruction, i) {
    jQuery('#exercise_instructions_output')
        .append('<textarea name="exercise_instructions['+i+'][description]" id="ingredient_description_'+i+'">'+instruction.text+'</textarea><input type="hidden" name="exercise_instructions['+i+'][group]" class="instructions_group" id="instruction_group_'+i+'" value="'+ImportText.escapeAttr(instruction.group)+'">');
};
