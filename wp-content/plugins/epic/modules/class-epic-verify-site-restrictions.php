...
class epic_Verify_Site_Restrictions{
    ...
    function __construct(){
        ...
        add_action('template_redirect', array($this, 'epic_verify_site_lockdown_restrictions'));
        add_filter('the_content',array($this,'epic_restrict_rss_feed' ));

        $this->current_option = get_option('epic_options');
        
        if($this->current_option['site_lockdown_status'] == '0'){
            add_action('template_redirect', array($this, 'epic_verify_site_content_restrictions'));
        }
        
    }

    function epic_verify_site_lockdown_restrictions(){
        global $epic,$pagenow;
    ...
    function epic_verify_site_content_restrictions(){
        global $epic,$pagenow;
        ...
        $restriction_rules = (array) get_option('epic_site_restriction_rules'); 
                ...
                if(is_array($restriction_rules)){                    
                    ...
                    foreach ($restriction_rules as $key => $rule) {
                        ...
                        if($rule['site_restriction_rule_status'] == '1'){
                            ...
                            $display_status = $this->epic_process_restriction_rules($rule);
    ...
    function epic_process_restriction_rules($rule){

        switch ($rule['site_content_user_restrictions']) {
            case 'by_all_users':
                return $this->epic_process_allow_part_loggedin_users($rule);
                break;
            
            case 'by_user_roles':
                return $this->epic_process_allow_part_user_roles($rule);
                break;
        }
    }

    function epic_process_allow_part_loggedin_users($rule){


        $content_status = $this->epic_process_content_locks($rule);
    ...
    function epic_process_allow_part_user_roles($rule){
        global $epic_roles;

        // Role status becomes true when role is in the restricted list
        $role_status = false;
      
        $current_user_roles = (array) $epic_roles->epic_get_user_roles_by_id($this->user_id);
    ...
    function epic_process_content_locks($rule){
        switch ($rule['site_content_section_restrictions']) {
            case 'all_pages':
                return $this->epic_process_all_pages_restriction($rule);
                break;
            
            case 'all_posts':
                return $this->epic_process_all_posts_restriction($rule);
                break;

            case 'restrict_selected_pages':

                return $this->epic_process_specific_pages_restriction($rule);
                break;

            case 'restrict_selected_posts':

                return $this->epic_process_specific_posts_restriction($rule);
                break;
            
            case 'restrict_sub_selected_pages':

                return $this->epic_process_specific_sub_pages_restriction($rule);
                break;
            
            case 'restrict_sub_include_selected_pages':

                return $this->epic_process_specific_sub_parent_pages_restriction($rule);
                break;
            
            case 'restrict_posts_by_categories':

                return $this->epic_process_specific_category_restriction($rule);
                break;

        }
    }

    function epic_process_all_pages_restriction($rule){
    ...
    function epic_process_all_posts_restriction($rule){
    ...
    function epic_process_specific_pages_restriction($rule){
        global $post;
    ...
    function epic_process_specific_sub_pages_restriction($rule){
        global $post;
        ...
        if($recursive_status){
            $parent_page_ids = epic_top_ancestors(array());
            ...
            $parent_page_id = epic_is_subpage();
    ...
    function epic_process_specific_sub_parent_pages_restriction($rule){
        global $post;
    ...
    function epic_process_specific_posts_restriction($rule){
        global $post;
    ...
    function epic_process_specific_category_restriction($rule){
        global $post;
    ...
    function epic_restrict_rss_feed($content){
...
$epic_verify_site_restrictions = new epic_Verify_Site_Restrictions();