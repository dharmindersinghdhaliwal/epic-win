 ...
 *
 * Since this function uses {@see maxxfitness_apply_filters()}, the $id argument may contain sub-hook(s).
 *
 * @since 1.0.0
 *
 ...
 */
function maxxfitness_output( $id, $output ) {

	$args = func_get_args();
	$args[0] = $id . '_output';

	$output = call_user_func_array( 'maxxfitness_apply_filters', $args );

	if ( empty( $output ) )
		return;

	if ( maxxfitness_tt_is_html_dev_mode() )
 ...
 */
function maxxfitness_remove_output( $id ) {

	return maxxfitness_add_filter( $id . '_output', false );
 ...
 * @return string The output.
 */
function maxxfitness_open_markup( $id, $tag, $attributes = array() ) {

	global $_tempmaxxfitness_tt_selfclose_markup;
	...
	if ( ( $tag = call_user_func_array( 'maxxfitness_apply_filters', $args ) ) === null )
		return;
	...
	$output = call_user_func_array( 'maxxfitness_tt_render_action', $args );

		// Don't output the tag if empty, the before and after actions still run.
		if ( $tag )
			$output .= '<' . $tag . ' ' . call_user_func_array( 'maxxfitness_add_attributes', $attributes_args ) . ( maxxfitness_tt_is_html_dev_mode() ? ' data-markup-id="' . $id . '"' : null ) . ( $_tempmaxxfitness_tt_selfclose_markup ? '/' : '' ) . '>';

	// Set after action id.
	$args[0] = $id . ( $_tempmaxxfitness_tt_selfclose_markup ? '_after_markup' : '_prepend_markup' );

	$output .= call_user_func_array( 'maxxfitness_tt_render_action', $args );

	// Reset temp selfclose global to reduce memory usage.
	unset( $GLOBALS['_tempmaxxfitness_tt_selfclose_markup'] );
 ...
 *
 * This function is shortuct of {@see maxxfitness_open_markup()}. It should be used for self-closed HTML markup such as
 * images or inputs.
 *
 * @since 1.0.0
 *
 ...
 * @return string The output.
 */
function maxxfitness_selfclose_markup( $id, $tag, $attributes = array() ) {

	global $_tempmaxxfitness_tt_selfclose_markup;

	$_tempmaxxfitness_tt_selfclose_markup = true;
	$args = func_get_args();

	return call_user_func_array( 'maxxfitness_open_markup', $args );
 ...
 *
 * This function is similar to {@see maxxfitness_open_markup()}, but does not accept HTML attributes. The $id
 ...
 * @return string The output.
 */
function maxxfitness_close_markup( $id, $tag ) {

	// Stop here if the tag is set to false, the before and after actions won't run in this case.
	if ( ( $tag = maxxfitness_apply_filters( $id . '_markup', $tag ) ) === null )
		return;
 ...
 * @return bool Will always return true.
 */
function maxxfitness_modify_markup( $id, $markup, $priority = 10, $args = 1 ) {

	return maxxfitness_add_filter( $id . '_markup', $markup, $priority, $args );
 ...
 * @return bool Will always return true.
 */
function maxxfitness_remove_markup( $id, $remove_actions = false ) {

	if ( $remove_actions )
		return maxxfitness_add_filter( $id . '_markup', null );

	return maxxfitness_add_filter( $id . '_markup', false );
 ...
 * @return bool Will always return true.
 */
function maxxfitness_reset_markup( $id ) {
 ...
 * Wrap markup.
 *
 * This function calls {@see maxxfitness_open_markup()} before the opening markup and
 * {@see maxxfitness_close_markup()} after the closing markup.
 *
 * @since 1.0.0
 *
 * @param string $id               The markup ID.
 ...
 * @return bool Will always return true.
 */
function maxxfitness_wrap_markup( $id, $new_id, $tag, $attributes = array() ) {

	$args = func_get_args();
	unset( $args[0] );

	maxxfitness_tt_add_anonymous_action( $id . '_before_markup', array( 'maxxfitness_open_markup', $args ), 9999 );

	unset( $args[3] );

	maxxfitness_tt_add_anonymous_action( $id . '_after_markup', array( 'maxxfitness_close_markup', $args ), 1 );
 ...
 *
 * This function calls {@see maxxfitness_open_markup()} after the opening markup and
 * {@see maxxfitness_close_markup()} before the closing markup.
 *
 * @since 1.0.0
 *
 * @param string $id               The markup ID.
 ...
 * @return bool Will always return true.
 */
function maxxfitness_wrap_inner_markup( $id, $new_id, $tag, $attributes = array() ) {

	$args = func_get_args();
	unset( $args[0] );

	maxxfitness_tt_add_anonymous_action( $id . '_prepend_markup', array( 'maxxfitness_open_markup', $args ), 1 );

	unset( $args[3] );

	maxxfitness_tt_add_anonymous_action( $id . '_append_markup', array( 'maxxfitness_close_markup', $args ), 9999 );
 ...
 */
function maxxfitness_add_attributes( $id, $attributes = array() ) {
	...
	$attributes = call_user_func_array( 'maxxfitness_apply_filters', $args );

	return maxxfitness_esc_attributes( $attributes );
 ...
 * @return bool Will always return true.
 */
function maxxfitness_reset_attributes( $id ) {
 ...
 */
function maxxfitness_add_attribute( $id, $attribute, $value ) {

	$class = new maxxfitness_tt_Attributes( $id, $attribute, $value );
 ...
 * @return array All targeted markup attributes.
 */
function maxxfitness_replace_attribute( $id, $attribute, $value, $new_value = null ) {

	$class = new maxxfitness_tt_Attributes( $id, $attribute, $value, $new_value );
 ...
 */
function maxxfitness_remove_attribute( $id, $attribute, $value = null ) {
 ...
 * @ignore
 */
function maxxfitness_tt_is_html_dev_mode() {

	if ( defined( 'maxxfitness_HTML_DEV_MODE' ) )
		return maxxfitness_HTML_DEV_MODE;
