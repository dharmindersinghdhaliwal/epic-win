 ...
 */
final class maxxfitness_tt_Page_Compiler {
	...
	public function compile_page_styles() {
		...
		if ( !maxxfitness_get_component_support( 'wp_styles_compiler' ) || !get_option( 'maxxfitness_compile_all_styles', false ) || maxxfitness_tt_is_compiler_dev_mode() )
			return;

		if ( $styles = $this->compile_enqueued( 'style' ) )
			maxxfitness_compile_css_fragments( 'beans', $styles, array( 'version' => null ) );
	...
	public function compile_page_scripts() {
		...
		if ( !maxxfitness_get_component_support( 'wp_scripts_compiler' ) || !get_option( 'maxxfitness_compile_all_scripts', false ) || maxxfitness_tt_is_compiler_dev_mode() )
			return;

		if ( $scripts = $this->compile_enqueued( 'script' ) )
			maxxfitness_compile_js_fragments( 'beans', $scripts, array(
				'in_footer' => ( 'aggressive' === get_option( 'maxxfitness_compile_all_scripts_mode', 'aggressive' ) ) ? true : false,
				'version' => null
			) );
	...
	private function compile_enqueued( $type, $depedencies = false ) {
		...
		$set_dequeued_global = 'maxxfitness_dequeue_' . $type . 's';
		...
		foreach ( $depedencies as $id ) {
			...
			if ( !$args = maxxfitness_get( $id, $$set_global->registered ) )
				continue;
			...
			if ( $type == 'style' ) {
				...
				if ( $args->args != 'all' )
					$args->src = add_query_arg( array( 'maxxfitness_compiler_media_query' => $args->args ), $args->src );
	...
	public function dequeue_scripts() {
		...
		foreach ( $this->dequeued_scripts as $id => $src ) {
			...
			if ( !$args = maxxfitness_get( $id, $wp_scripts->registered ) )
				continue;
...
new maxxfitness_tt_Page_Compiler();