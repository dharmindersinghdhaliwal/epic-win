...
class WPUPG_Etemplate_Block  {
    ...
    private function get_style_string( $name )
    {
        ...
        {
            if( WPUltimateEPostGrid::option( 'grid_template_force_style', '1' ) == '1' ) {
    ...
    protected function style( $names = 'default' )
    {
        ...
        if( WPUltimateEPostGrid::option( 'grid_template_inline_css', '1' ) == '1' ) {
    ...
    private function condition( $post, $condition, $args = array() )
    {
        ...
        if( $condition['condition_type'] == 'field' ) {
            ...
            if( in_array( $condition['setting'], array(
                ...
                // Default 0
                $val = WPUltimateEPostGrid::option( $condition['setting'], '0' );
            } else {
                // Default 1
                $val = WPUltimateEPostGrid::option( $condition['setting'], '1' );
            }
    ...
    protected function before_output()
    {
        ...
        if( $this->background_preset )
        {
            switch( $this->background_preset ) {
                case 'default':
                    $img = WPUltimateEPostGrid::addon( 'custom-templates' )->addonUrl . '/img/default.png';
                    break;
                default:
                    $img = WPUltimateEPostGrid::addon( 'template-editor' )->addonUrl . '/img/' . $this->background_preset . '.png';
            }
    ...
    protected function after_output( $output, $post )
    {
        ...
        return apply_filters( 'wpupg_eoutput_grid_block_' . $this->type, $output, $post );
    }
