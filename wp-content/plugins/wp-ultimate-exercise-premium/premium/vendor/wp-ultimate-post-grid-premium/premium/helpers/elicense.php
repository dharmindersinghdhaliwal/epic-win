...
class WPUPG_Elicense {
    ...
    public function __construct()
    {
        ...
        define( 'EDD_WPUPG_STORE_EURL', 'http://bootstrapped.ventures' );
        define( 'EDD_WPUPG_EPRODUCT', 'WP Ultimate Post Egrid Premium' );
		
        if( !class_exists( 'EDD_SL_Plugin_Updater' ) ) {
            include( WPUltimateEPostGridPremium::get()->premiumDir . '/vendor/EDD/EDD_SL_Plugin_Updater.php' );
        }
    }

    public function license_notice()
    {
        if( get_current_screen()->id == WPUPG_EPOST_TYPE . '_page_wpupg_eadmin' ) {
            $license 	= get_option( 'edd_wpupg_license_key' );
            $status 	= get_option( 'edd_wpupg_license_status' );

            include( WPUltimateEPostGridPremium::get()->premiumDir . '/helpers/license_form.php' );
        }
    }
    ...
    public function plugin_updater() {
        ...
        // setup the updater
        $edd_updater = new EDD_SL_Plugin_Updater( EDD_WPUPG_STORE_EURL, WPUltimateEPostGrid::get()->pluginFile, array(
                'version' 	=> WPUPG_EPREMIUM_VERSION, 				// current version number
                'license' 	=> $license_key, 		// license key (used get_option above to retrieve from DB)
                'item_name' => EDD_WPUPG_EPRODUCT, 	// name of this plugin
    ...
    public function activate_license( $key ) {
        ...
        $api_params = array(
            ...
            'license' 	=> $license,
            'item_name' => urlencode( EDD_WPUPG_EPRODUCT ), // the name of our product in EDD
            'url'       => home_url()
        );

        // Call the custom API.
        $response = wp_remote_get( add_query_arg( $api_params, EDD_WPUPG_STORE_EURL ), array( 'timeout' => 60, 'sslverify' => false ) );
    ...
    public function edd_wpupg_check_license() {
        ...
        $api_params = array(
            ...
            'license' => $license,
            'item_name' => urlencode( EDD_WPUPG_EPRODUCT ),
            'url'       => home_url()
        );
