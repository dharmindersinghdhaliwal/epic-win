 ...
 */
function maxxfitness_render_function( $callback ) {
 ...
 * @return string The callback content.
 */
function maxxfitness_render_function_array( $callback, $params = array() ) {
 ...
 */
function maxxfitness_remove_dir( $dir_path ) {
 ...
 * @return string Url.
 */
function maxxfitness_path_to_url( $path ) {
	...
	if ( !$root ) {
		...
		$explode = maxxfitness_get( 0, explode( '/' , trailingslashit( ltrim( $subfolder, '/' ) ) ) );
	...
	elseif ( stripos( $path, maxxfitness_get( 'DOCUMENT_ROOT', $_SERVER ) ) !== false )
		$path = str_replace( maxxfitness_get( 'DOCUMENT_ROOT', $_SERVER ), '', $path );
...
function maxxfitness_url_to_path( $url ) {
	...
	$explode = maxxfitness_get( 0, explode( '/' , trailingslashit( ltrim( $path, '/' ) ) ) );
	...
	if ( isset( $set_root ) )
		$root = maxxfitness_sanitize_path( $root );

	// Add root of it doesn't exist.
	if ( strpos( $path, $root ) === false )
		$path = trailingslashit( $root ) . ltrim( $path, '/' );

	return maxxfitness_sanitize_path( $path );
 ...
 */
function maxxfitness_sanitize_path( $path ) {
 ...
 */
function maxxfitness_get( $needle, $haystack = false, $default = null ) {
 ...
 * @return string Value if found, $default otherwise.
 */
function maxxfitness_post( $needle, $default = null ) {

	return maxxfitness_get( $needle, $_POST, $default );
 ...
 * @return string Value if found, $default otherwise.
 */
function maxxfitness_get_or_post( $needle, $default = null ) {

	if ( $get = maxxfitness_get( $needle ) )
		return $get;

	if ( $post = maxxfitness_post( $needle ) )
		return $post;
 ...
 */
function maxxfitness_count_recursive( $array, $depth = false, $count_parent = true ) {
	...
	foreach ( $array as $_array )
		 if ( is_array( $_array ) )
			$count += maxxfitness_count_recursive( $_array, $depth - 1, $count_parent );
		 else
			$count += 1;
 ...
 * @param bool   $strict   If the third parameter strict is set to true, the maxxfitness_in_multi_array()
 *                         function will also check the types of the needle in the haystack.
 *
 * @return bool True if needle is found in the array, false otherwise.
 */
function maxxfitness_in_multi_array( $needle, $haystack, $strict = false ) {

	if ( in_array( $needle, $haystack, $strict ) )
		return true;

	foreach ( (array) $haystack as $value )
		if ( is_array( $value ) && maxxfitness_in_multi_array( $needle , $value ) )
			return true;
 ...
 * @return bool True if needle is found in the array, False otherwise.
 */
function maxxfitness_multi_array_key_exists( $needle, $haystack ) {

	if ( array_key_exists( $needle, $haystack ) )
		return true;

	foreach ( $haystack as $value )
		if ( is_array( $value ) && maxxfitness_multi_array_key_exists( $needle , $value ) )
			return true;
 ...
 */
function maxxfitness_array_shortcodes( $content, $haystack ) {
	...
	if ( preg_match_all( '#{(.*?)}#', $content, $matches ) ) {
		...
		foreach ( $matches[1] as $needle ) {
			...
			foreach ( $sub_keys as $sub_key ) {
				...
				$value = maxxfitness_get( $sub_key, $search );
 ...
 */
function maxxfitness_admin_menu_position( $position ) {
	...
	if ( array_key_exists( $position, $menu ) )
		return maxxfitness_admin_menu_position( $position + 1 );
 ...
 */
function maxxfitness_esc_attributes( $attributes ) {
	 ...
	 */
	$methods = apply_filters( 'maxxfitness_escape_attributes_methods', array(
		'href' => 'esc_url',
		'src' => 'esc_url',
		'itemtype' => 'esc_url',
		'onclick' => 'esc_js'
	...
	foreach ( (array) $attributes as $attribute => $value ) {
		...
		if ( $value !== null ) {
			...
			if ( $method = maxxfitness_get( $attribute, $methods ) )
...
if ( !function_exists( 'array_replace_recursive' ) ) {
	...
	function array_replace_recursive( $base, $replacements )  {
		...
		foreach ( $replacements as $key => $value ) {
			...
			if ( is_array( $value ) && is_array( $from_base = maxxfitness_get( $key, $base ) ) )
