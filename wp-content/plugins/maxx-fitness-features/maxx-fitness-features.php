<?php
/*
  Plugin Name: MaxxFitness Features widget
  Plugin URI: http://themeforest.net/user/torbara/?ref=torbara
  Description: MaxxFitness Features widget
  Author: Torbara
  Version: 3.0.0
  Author URI: http://themeforest.net/user/torbara/portfolio/?ref=torbara
 */

class MaxxFitnessFeatures extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'MaxxFitnessFeatures', 'description' => 'Displays a Features posts with thumbnail');
        parent::__construct('MaxxFitnessFeatures', 'MaxxFitness Features', $widget_ops);
    }

    function form($instance) { ?>

        <?php $title = isset( $instance['title']) ? esc_attr( $instance['title'] ) : "MaxxFitness Features"; ?>
        <p>
        ...
        <div class="category-module-features">
            <div class="uk-grid" data-uk-grid-margin="" data-uk-grid-match="{target:'.uk-panel'}">
                <?php foreach ($list as $item) : ?>
                    ...
                    ?>
                    <div class="uk-width-medium-1-2 uk-width-large-1-<?=esc_attr($widthLarge)?> uk-text-center" data-uk-scrollspy="{cls:'uk-animation-scale-up', repeat: true}">
                        <div class="uk-overlay tm-overlay uk-width-1-1">
                            <div class="uk-panel uk-panel-box">
                                <?php $thumbnail_object = get_post(get_post_thumbnail_id($item->ID)); ?>
                                <?php if($thumbnail_object->guid) {?><img src="<?php echo esc_attr($thumbnail_object->guid); ?>" alt="<?=esc_attr($item->post_title)?>"><?php }?>
                                <h2 class="uk-panel-title uk-margin-top"><?=esc_html ($item->post_title)?></h2>
                                <p class="tm-panel-subtitle"><?php echo esc_html(strip_tags(wp_trim_words($item->post_content, 20 ))); ?></p>
                            </div>
                            <div class="uk-overlay-area">
                                <div class="uk-overlay-area-content">
                                    <p><?php echo esc_html(strip_tags(wp_trim_words($item->post_content, 50 ))); ?></p>
                                </div>
                            </div>
                        </div>
                        <a class="uk-button uk-button-large" href="<?php echo esc_url ($item->guid); ?>" title="View <?=esc_attr($item->post_title)?>">
                            <i class="uk-icon-akplus">+</i>
                            <span><?php echo esc_html("View ".$item->post_title); ?></span>
                        </a>
                    </div>
                <?php endforeach; ?>
            </div>
...
//add_action('widgets_init', create_function('', 'return register_widget("MaxxFitnessFeatures");'));
add_action('widgets_init', 'MaxxFitnessFeatures' );

function MaxxFitnessFeatures()
{
    return register_widget('MaxxFitnessFeatures');
}